...
const RingsDynamicForm = ({ onSelectedSizes, sizes , ringsSizes}) => {
  const [fields, setFields] = useState([{ size: "", quantity: "", sku: "" }]);

  useEffect(() => {
    const savedFields = localStorage.getItem("ring");
    if (savedFields) {
  ...
  const handleChange = (index, event) => {
    ...
    localStorage.setItem("ring", JSON.stringify(values));
  };
  ...
  const addField = () => {
    ...
    setFields(newFields);
    localStorage.setItem("ring", JSON.stringify(newFields));
  };
...
export default RingsDynamicForm;
